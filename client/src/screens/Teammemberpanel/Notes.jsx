...
export default function Notes() {
    ...
    // Team members can only delete or edit their own notes
            ...
            {loading ? (
                ...
                <div className='w-full'>
                    <div className="flex flex-col md:flex-row">
                        {/* We use Teamnavbar on top/left depending on how it's styled. Usually it's top or left */}
                                            ...
                                            <div key={note._id} className="card-standard p-6 transition-all hover:shadow-md border-l-4 border-primary">
                                                <div className="flex justify-between items-start mb-4">
                                                    <div className="flex items-center gap-3">
                                                        <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold uppercase ${note.authorModel === 'user' ? 'bg-indigo-100 text-indigo-700' : 'bg-blue-100 text-primary'}`}>
                                                            {note.authorName.charAt(0)}
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-gray-800">{note.authorName} <span className={`text-xs font-normal ml-2 py-0.5 px-2 rounded-full ${note.authorModel === 'user' ? 'bg-indigo-50 text-indigo-600' : 'bg-gray-100 text-gray-500'}`}>{note.authorModel === 'user' ? 'Owner' : 'Team'}</span></p>
