...
export default function Editinventory() {
  ...
  const fetchdata = async () => {
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/inventory/${itemId}`, {
        headers: {
      ...
      if (response.status === 401) {
        ...
        setEditorData(json.description || "<p></p>");
        setloading(false);
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    const authToken = localStorage.getItem('authToken');
    try {
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/editinventory/${itemId}`, {
        method: 'POST',
        headers: {
      ...
      if (response.status === 401) {
        ...
        if (json.success) {
          ...
          navigate('/userpanel/InventoryList');
            ...
            setAlertMessage(json.message || "Failed to update inventory.");
            setloading(false);
        }
      }
    } catch (error) {
                ...
                <div className="bg-white my-5 p-6 card-standard mx-4">
                        ...
                        <div className="flex flex-wrap">
                          <div className="w-full lg:w-1/2 px-2 mb-6">
                            ...
                            <input
                              type="text"
                              name="materialName"
                              value={credentials.materialName}
                  ...
                  <div className="flex flex-wrap -mx-2 pt-4 pe-2">
                    ...
                    <div className="col-4 col-sm-2 text-right">
                      <button onClick={(e) => handleSubmit(e)} className="btn btnclr text-white">Save Changes</button>
                    </div>
                  </div>
                </div>
              </div>
