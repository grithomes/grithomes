...
.text-end {
  ...
  const fetchinvoiceData = async () => {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/getemailinvoiceData/${invoiceId}`, {
        // headers: {
  ...
  const fetchtransactiondata = async () => {
    try {
      // const userid = localStorage.getItem("userid");
      // const authToken = localStorage.getItem('authToken');
      const response = await fetch(`http://localhost:3000/api/getemailtransactiondata/${invoiceId}`, {
        // headers: {
        //   'Authorization': authToken,
        // }
      });
  ...
  const checkCustomerSignature = async (invoiceIdpass) => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/checkcustomersignatureusinginvoice/${encodeURIComponent(invoiceIdpass)}`);
      const json = await response.json();
  ...
  const handleSignatureClick = async () => {
    ...
    try {
      ...
      const checkResponse = await fetch(`http://localhost:3000/api/checkcustomersignatureusinginvoice/${encodeURIComponent(invoiceData._id)}`);
      const checkJson = await checkResponse.json();
  
      if (checkJson.hasSignature) {
        // Update the existing customer signature
        const updateResponse = await fetch(`http://localhost:3000/api/updatecustomersigninv/${encodeURIComponent(invoiceData._id)}`, {
          method: 'PUT',
          headers: {
...
//     try {
//         const response = await fetch('http://localhost:3000/api/customersignature', {
//             method: 'POST',
//             headers: {
//                 'Content-Type': 'application/json',
//                 // 'Authorization': authToken,
...
// };
  ...
  const handleDocumentComplete = async () => {
    try {
      ...
      const emailResponse = await fetch('http://localhost:3000/api/send-Invoice-signed-email', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  //     // Send email request to backend
  //     const response = await fetch('http://localhost:3000/api/send-Invoice-signed-email', {
  //       method: 'POST',
  //       headers: {
  //         'Content-Type': 'application/json',
  //       },
