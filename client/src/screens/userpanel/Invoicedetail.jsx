  ...
  const handleAddPayment = async () => {
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        await fetch(`http://localhost:3000/api/updateinvoicedata/${invoiceid}`, {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': authToken,
...
.text-end {
  ...
  const handleDeleteTransClick = async (transactionid) => {
    try {
        const authToken = localStorage.getItem('authToken');
        const response = await fetch(`http://localhost:3000/api/deltransaction/${transactionid}`, {
            method: 'GET',
            headers: {
                'Authorization': authToken,
              }
...
const handleRemove = async (invoiceid, invoiceIdpass) => {
  try {
    ...
    if (signatureData) {
      const authToken = localStorage.getItem('authToken');
      const deleteSignatureResponse = await fetch(`http://localhost:3000/api/delcustomersignature/${encodeURIComponent(invoiceIdpass)}`, {
        method: 'DELETE',
        headers: {
          'Authorization': authToken,
        }
    ...
    const authToken = localStorage.getItem('authToken');
    const response = await fetch(`http://localhost:3000/api/deldata/${invoiceid}`, {
      method: 'GET',
      headers: {
        'Authorization': authToken,
      }
