...
export default function Editinvoice() {
    ...
    const fetchInvoiceData = async () => {
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/geteditinvoicedata/${invoiceid}`, {
                headers: {
    ...
    const handleSaveClick = async () => {
        try {
            ...
            const authToken = localStorage.getItem('authToken');
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/updateinvoicedata/${invoiceid}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': authToken,
    ...
    const handleDeleteClick = async (itemId) => {
        try {
            ...
            const authToken = localStorage.getItem('authToken');
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/delinvoiceitem/${invoiceData._id}/${itemId}`, {
                method: 'GET',
                headers: {
                    'Authorization': authToken,
                }
                            ...
                            <div className="flex-1 w-full mx-auto px-4">
                                ...
                                <div className='mx-4'>
                                    ...
                                    <div className='flex flex-wrap items-center justify-between py-6 px-4 mb-6 bg-white shadow-sm rounded-xl border border-gray-100'>
                                        <div>
                                            <p className='text-3xl font-bold text-gray-800'>Edit Invoice</p>
                                            <nav aria-label="breadcrumb">
                                                <ol className="flex text-sm text-gray-500 mt-2 space-x-2">
                                                    ...
                                                    <li><a href="/userpanel/Invoice" className='hover:text-primary transition-colors text-decoration-none'>Invoice</a></li>
                                                    <li><span className="mx-2">/</span></li>
                                                    <li className="text-gray-800 font-semibold" aria-current="page">Edit Invoice</li>
                                                </ol>
                                            </nav>
                                        </div>
                                        ...
                                        <div className="lg:w-3/4 w-full order-2 lg:order-1">
                                                ...
                                                <div className='flex flex-col md:flex-row gap-8'>
                                                    <div className="md:w-1/3">
                                                        <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 h-full">
                                                            ...
                                                            <ul>
                                                                <li className='font-semibold text-lg text-primary'>{invoiceData.customername}</li>
                                                            </ul>
                                                            <p className="text-gray-600 mt-2">{invoiceData.customeremail}</p>
                                                        </div>
                                                    </div>
                                                    <div className="md:w-2/3">
                                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                            <div>
                                                                <label htmlFor="invoicenumbr" className="block text-sm font-medium text-gray-700 mb-1">Invoice Number</label>
                                                                <input type="text" name="InvoiceNumber" className="input-standard bg-gray-100 cursor-not-allowed" value={invoiceData.InvoiceNumber} onChange={onchange} id="invoicenumbr" required disabled />
                                                            </div>
                                                            <div>
                                                                <label htmlFor="purchaseoder" className="block text-sm font-medium text-gray-700 mb-1">Purchase Order (PO) #</label>
                                                                <input type="text" name="purchaseorder" className="input-standard" value={invoiceData.purchaseorder} onChange={onchange} id="purchaseoder" />
                                                            </div>
                                                            <div>
                                                                <label htmlFor="Date" className="block text-sm font-medium text-gray-700 mb-1">Date</label>
                                                                <input type="date" name="date" className="input-standard" value={new Date(invoiceData.date).toISOString().split('T')[0]} onChange={onchange} id="Date" required />
                                                            </div>
                                                            <div>
                                                                <label htmlFor="job" className="block text-sm font-medium text-gray-700 mb-1">Job</label>
                                                                <input type="text" name="job" className="input-standard" value={invoiceData.job} onChange={onchange} id="job" />
                                                            </div>
                                                            <div>
                                                                <label htmlFor="duedate" className="block text-sm font-medium text-gray-700 mb-1">Due Date</label>
                                                                <input type="date" name="duedate" className="input-standard" value={new Date(invoiceData.duedate).toISOString().split('T')[0]} onChange={onchange} id="duedate" />
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className='card-standard mb-8'>
                                                ...
                                                <div className="divide-y divide-gray-50">
                                                        ...
                                                        <div className='py-4' key={item.itemId}>
                                                            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
                                                                ...
                                                                <div className="col-span-1 md:col-span-2">
                                                                    ...
                                                                    <input type="text" name="price" className="input-standard" value={item.price} id={`price-${item.itemId}`} required onChange={(event) => handlePriceChange(event, item.itemId)} onBlur={(event) => handlePriceBlur(event, item.itemId)} />
                                                                </div>
                                                    ...
                                                    return (
                                                        <div className='flex flex-wrap -mx-2'  key={item.itemId}>
                                                            <div className="col-6 ">
                                                                <div className="mb-6 flex align-items-baseline justify-between">
                                                                    <p>{item.label}</p>
                                                                    <button type="button" className="btn btn-danger btn-sm mr-2" onClick={() => onDeleteItem(item.value)}>
                                                ...
                                                <div className="flex flex-col md:flex-row gap-8 pt-6 border-t border-gray-100">
                                                    <div className="md:w-1/2">
                                                        <div className="search-container forms">
                                                            <p className='text-sm font-medium text-gray-700 mb-2'>Select Item to Add</p>
                                                            <Select className="z-10" value={searchitemResults} onChange={onChangeitem} options={items.map(item => ({ value: item._id, label: item.itemname }))} placeholder="Search item..." />
                                                        </div>
                                                    </div>
                                                    <div className="md:w-1/2 bg-gray-50 p-6 rounded-xl border border-gray-100">
                                                        <div className="space-y-3 text-gray-700">
                                                            <div className="flex justify-between">
                                                                <p>Subtotal</p>
                                                                <p className="font-medium"><CurrencySign />{calculateSubtotal().toLocaleString('en-IN')}</p>
                                                            </div>
                                                            <div className="flex justify-between items-center">
                                                                <p>GST {invoiceData.taxpercentage}%</p>
                                                                <p className="font-medium"><CurrencySign />{calculateTaxAmount().toLocaleString('en-IN')}</p>
                                                            </div>
                                                            <div className="flex justify-between items-center">
                                                                <p>Discount</p>
                                                                <div className="w-32">
                                                                    <input type="number" name="totaldiscount" className="input-standard py-1 px-2 text-right" value={discountTotal} onChange={handleDiscountChange} placeholder="0" id="discountInput" min="0" />
                                                                </div>
                                                            </div>
                                                            <div className="flex justify-between border-t border-gray-200 pt-3 mt-3 text-lg font-bold text-gray-900">
                                                                <p>Total Amount Due</p>
                                                                <p><CurrencySign />{calculateTotal().toLocaleString('en-IN')}</p>
                                                            </div>
                                                        </div>
                                                    </div>
                                                </div>
                                            ...
                                            <div className='card-standard mt-8'>
                                                <h3 className="text-lg font-semibold text-gray-800 mb-4">Notes & Information</h3>
                                                <CKEditor editor={ClassicEditor} data={invoiceData.information} config={{ extraPlugins: [MyCustomUploadAdapterPlugin] }} onChange={handleEditorChange} />
                                            </div>
                                        </div>

                                        <div className="lg:w-1/4 w-full order-1 lg:order-2">
                                            <div className='card-standard mb-8 bg-gray-50'>
                                                <h3 className="text-lg font-semibold text-gray-800 mb-4">Settings</h3>
                                                <div className="space-y-4">
                                                    <div className="flex items-center justify-between">
                                                        <label className="text-sm font-medium text-gray-700" htmlFor="signatureSwitch">Enable Signature</label>
                                                        <input className="form-check-input w-10 h-5" type="checkbox" role="switch" id="signatureSwitch" onChange={handleSignatureSwitch} checked={hasSignature} />
                                                    </div>
                                                    {hasSignature && (
                                                        <div className="pl-4 border-l-2 border-primary space-y-3 mt-3">
                                                            <div className="flex items-center justify-between">
                                                                <label className="text-sm text-gray-600" htmlFor="addSignatureSwitch">Add My Signature</label>
                                                                <input className="form-check-input w-8 h-4" type="checkbox" role="switch" id="addSignatureSwitch" checked={isAddSignatureSwitchOn} onChange={handleAddSignatureSwitch} />
                                                            </div>
                                                            <div className="flex items-center justify-between">
                                                                <label className="text-sm text-gray-600" htmlFor="customerSignSwitch">Customer to Sign</label>
                                                                <input className="form-check-input w-8 h-4" type="checkbox" role="switch" id="customerSignSwitch" checked={isCustomerSignSwitchOn} onChange={handleCustomerSignSwitch} />
                                                            </div>
                                                        </div>
                                                    )}
                                                </div>
